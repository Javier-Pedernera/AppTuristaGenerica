...
const TouristDetailScreen: React.FC<TouristDetailScreenProps> = ({ route, navigation }) => {
  ...
  const [selectedImage, setSelectedImage] = useState<string | null>(touristPoint.images[0]?.image_path || null);
  ...
  const handleUpdateRating = () => {
    if (editingRatingId !== null) {
      ...
      dispatch(updateExistingRating(editingRatingId, updatedRating));
        ...
        {selectedImage && (
          ...
          <View style={styles.thumbnailContainer}>
            {touristPoint.images && touristPoint.images.length > 1 && touristPoint.images.map(image => (
              <TouchableOpacity key={image.id} onPress={() => setSelectedImage(image.image_path)}>
                <Image source={{ uri: image.image_path }} style={styles.thumbnail} />
              </TouchableOpacity>
            ))}
          </View>
