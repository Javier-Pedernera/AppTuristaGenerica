...
const PromotionDetailScreen: React.FC = () => {
  ...
  const promoImage = promotion && promotion.images.length > 0 ? promotion.images[0].image_path : 'https://res.cloudinary.com/dbwmesg3e/image/upload/v1721157537/TurismoApp/no-product-image-400x400_1_ypw1vg_sw8ltj.png';

  // console.log("ratings en descripcion ",ratings);
  // console.log("branch en descripcion ",branch);
  console.log("ruta de descripcion ",route)
  useEffect(() => {
    if (promotionId) {
  ...
  const handleShare = async () => {
    try {
      // Crea la URL usando el esquema https y el dominio correspondiente
      const url = `https://app-cobquecura.vercel.app/PromotionDetail/${promotion?.promotion_id}`;
      console.log("url compartida", url);
  
      await Share.share({
        message: `Mira esta promoción: ${url}`,
        url: url
      });
